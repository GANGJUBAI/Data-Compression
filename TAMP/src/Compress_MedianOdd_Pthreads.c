...
double *data,*datatrans,*C;
int thread_count;

void *BulidNewCube(void* rank){
	...
	for(i=my_first_row;i<my_last_row+1;i++){
        	for(j=0;j<cols;j++){
                	for(int k=0;k<layers;k++)
                        	datatrans[k*rows*cols+i*cols+j]=data[k*rows*cols+i*cols+j]-C[i*cols+j];
...
void* CreateCMP(void* rank){
      ...
      // printf("this process %ld running \n",my_rank);
       ...
       for(i=my_first_row;i<my_last_row+1;i++){
				...
				C[i*cols+j]= Median_Odd(delta_X,layers);
                        ...
                        else
                        {
                                ...
                                double median=Median_Odd(delta_X,layers);
...
int main(int argc, char* argv[])
{
	...
	printf("CMP time is %fms\n",delta_t*1000);	
	

	datatrans =(double *)malloc(dataLength*sizeof(double));
        
        gettimeofday( &t_start, NULL);
        
	for(thread=0; thread<thread_count;thread++){
		 pthread_create(&thread_handles[thread],NULL,BulidNewCube,(void*)thread);
	}
	for(thread=0; thread<thread_count;thread++){
  		pthread_join(thread_handles[thread],NULL);
	}
