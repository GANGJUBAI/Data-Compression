...
int main(int argc, char* argv[])
{
	...
	int c_size_fpc=FPC_Compress(8,fp);
	//printf("%d\n",dataLength);
	fclose(fp);
	...
	printf("fpzip zorder compression ratio=%lf\n",(double)dataLength*sizeof(double)/c_size_fpzip);
	...
	if(fp==NULL){
	 ...
	 c_size_fpc=FPC_Compress(8,fp);
	fclose(fp);
	printf("fpc zorder compression ratio=%lf\n",(double)dataLength*sizeof(double)/c_size_fpc);



	c_size_fpzip=fpzip_double_array(data_hilbert, dataLength, 1, 1, 64);
	printf("fpzip hilbert compression ratio=%lf\n",(double)dataLength*sizeof(double)/c_size_fpzip);
	...
	c_size_fpc=FPC_Compress(8,fp);
	fclose(fp);
	printf("fpc hilbert compression ratio=%lf\n",(double)dataLength*sizeof(double)/c_size_fpc);
