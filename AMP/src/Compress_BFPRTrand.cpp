...
int main(int argc, char* argv[])
{
	...
	printf("fpc+/fpzip+ BFPRT median time is %fms\n",endtime);
	...
	printf("fpc+/fpzip+ with BFPRT precondition time is %fms\n",endtime);
	...
	gettimeofday( &end, NULL);
	endtime=(double)1000 * (end.tv_sec - start.tv_sec) + ((end.tv_usec - start.tv_usec) / 1000.0);
	printf("fpc ori+ with BFPRT compress time is %fms\n",endtime);
	fclose(fp);
	fp=NULL;
	...
	c_size_fpzip=fpzip_double_array(data, dataLength, 1, 1, 64);
        gettimeofday( &end, NULL );
	endtime=(double)1000 * (end.tv_sec - start.tv_sec) + ((end.tv_usec - start.tv_usec) / 1000.0);
	printf("fpzip ori+ with BFPRT compress time is %fms\n",endtime);
