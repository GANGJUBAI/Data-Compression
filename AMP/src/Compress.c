...
int main(int argc, char* argv[])
{
	...
	//rewind(ori_fp);    //point to the begining of ori_fp
	...
	fp=fopen("./median_value/temp_h","w");
	if(fp==NULL){
	...
	fp=fopen("./median_value/temp_h","r");
	if(fp==NULL){
		printf("Cannot open file %s.fpc\n",oriFilePath);
		return 0;
	}
	...
	fp=fopen("./median_value/temp_z","w");
	if(fp==NULL){
		printf("Cannot open file %s.fpc\n",oriFilePath);
		return 0;
	}
	...
	fp=fopen("./median_value/temp_z","r");
	if(fp==NULL){
		printf("Cannot open file %s.fpc\n",oriFilePath);
		return 0;
	}
