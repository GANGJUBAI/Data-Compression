...
*/
	...
	double *delta_X  =(double *)malloc(layers*sizeof(double));
        ...
        for(i=0;i<rows;i++)
			...
			//	if(i==190&&j==0)
                        //           printf("delta_X[0] is %lf,C[190,0] is %lf\n",delta_X[0],C[i*cols+j]);
                        ...
                        else
                        {
                                for(int k=0;k<layers;k++){
                                    ...
                                    //    ans=SignedOverflowsub(data[k*sizeN+i*cols+j-1], data[k*sizeN+i*cols+j]);
				//	if(ans==-1)
				//	    deltaflow++;
                                }
                                qsort(delta_X, layers, sizeof(double), cmpfunc);
				...
				//ans=SignedOverflowsub(C[i*cols+j-1], median);
				//if(ans==-1)
                                  //          Cflow++;
                                //printf("%lf\n", C[i*cols+j]);
	...
	printf("time is %fms\n",endtime*1000);
       ...
       //printf("deltaflow is %d",deltaflow);
      // printf("Cflow is %d",Cflow);
